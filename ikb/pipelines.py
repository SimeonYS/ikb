...
class IkbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ikb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ikb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ikb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ikb (date, title, link, content)"
